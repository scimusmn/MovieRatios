include(['./webcam.js','./canvas.js','src/playback.js','src/bgCanvas.js','src/frameCanvas.js','src/overlayDiv.js'], function() {
  function app() {};

  var live = µ('#live');        //create the live feed canvas object
  var last = µ('#lastFrame');      //create the canvas object to display the previous frame
  ...
  //initializing function: this is run at the application start to set the refresh timer and resize the app
    ...
    setTimeout(function () {book.drawFrame(last.ctx,0,0);},1000);
    };

  //function which is called when the playback is stopped.
    app.stopCB = function(){
      µ('#playLay').show();      //display the playback overlay.
    }
  ...
  //else, if the reset movie button has been pressed, hide the overlay if any button besides the reset button is pressed
    else if(e.which!=charCode('N')&&µ('#delLay').display) µ('#delLay').hide();
  //else, if the movie just finished playing, if any button besides play is pressed,
    else if(e.which!=charCode('P')&&µ('#playLay').display){
      µ('#playLay').hide();              //hide the play overlay
  ...
  //if none of the above are true, then...
      ...
      case charCode('N'):          //if the reset button was pressed
        if(µ('#delLay').display){      //if the reset overlay is showing
          book.clear();      //clear the frames from the flipbook object
          µ('#delLay').hide();      //hide the reset overlay
          updateNumFrames(0);
        }
        else µ('#delLay').show();      //if the reset overlay was not showing, display it
        break;
      ...
      case charCode('P'):              //if the play button was pressed
        if(µ('#playLay').display) µ('#playLay').hide();  //if the play overlay was showing, hide it
