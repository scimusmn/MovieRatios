obtain(['µ/canvas.js', './src/flipBookPixels.js'], function(cnv, fbp) {
  ...
  exports.playback = function(bk) {
    ...
    var idle = function() {
      if (playing && pos < _this.book.length() - 1) {  //if the current frame number is less than the number of frames
        //put imageData into the dummy canvas at normal size
        dCtx.putImageData(_this.book.frame(pos), 0, 0);
        ...
        pos++;
      } else if (_this.book.length() - 1 <= pos) { //if the current frame number is >= the number of frames
        _this.stop();          //stop playback
