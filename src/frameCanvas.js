obtain(['µ/canvas.js'], function(cnv) {
  exports.FrameStrip = function() {
    console.log('here');
    var _this = this;
    var strip = µ('#strip');              //declare the frame strip canvas
    var temp = document.createElement('canvas', { is: 'can-vas' });    //create a temporary canvas to draw image data to.
    ...
    this.draw = function() {
      ...
      if (Math.abs(slide) >= Math.abs(slideInc)) {    //if 'slide' is greater than 4,
        ...
        setTimeout(_this.draw.bind(_this), 20);  //and set draw to be called again in 20 milliseconds
